...
const memory = globalThis.__BLAKE_CACHE_V5__ || (globalThis.__BLAKE_CACHE_V5__ = new Map());
...
function collectorFamily(row){
  ...
  if(set.includes("pokemon tcg classic")) return "tcg classic";
  return "";
}
...
async function fetchJson(url) {
  ...
  for(let i=0;i<4;i++){
    try{
      const r=await fetch(url,{headers:{"User-Agent":"BlakeCardPricer/5.0"}});
...
function confidence(best){
  ...
  if(exactName && setScore>=.96 && (exactNum || numberMissing)) return "Exact Match";
  ...
  if(score>=.82&&exactName&&setScore>=.88) return "Strong Automatic Match";
  ...
  try{
    ...
    for(const row of rows){
      ...
      results.push({...row,
        ...
        "Review Notes": tier!=="Needs Review"
